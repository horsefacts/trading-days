...
contract TradingDaysTest is Test, Deployers, GasSnapshot {
    ...
    TradingDays tradingDays = TradingDays(
    ...
    function setUp() public {
        ...
        unchecked {
            for (uint256 i = 0; i < writes.length; i++) {
                ...
                vm.store(address(tradingDays), slot, vm.load(address(impl), slot));
            }
        }

        // Create the pool
        poolKey = IPoolManager.PoolKey(Currency.wrap(address(token0)), Currency.wrap(address(token1)), 3000, 60, IHooks(tradingDays));
    ...
    function test_SubsequentSwapsDoNotRingOpeningBell() public {
        ...
        assertEq(entries[0].topics[0], keccak256("Swap(bytes32,address,int128,int128,uint160,uint128,int24,uint24)"));
